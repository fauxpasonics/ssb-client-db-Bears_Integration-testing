...
CREATE PROC [MERGEPROCESS_New].[MergeDetect_MSCRM] --'bears'
    @Client VARCHAR(100) 
AS
Declare @Date Date = (select cast(getdate() as date));
DECLARE @Account VARCHAR(100) = (SELECT CASE WHEN @client = 'bears' THEN 'CRM_Account' ELSE CONCAT(@client,' PC_SFDC Account' ) END);--updateme
DECLARE @Contact VARCHAR(100) = (SELECT CASE WHEN @client = 'bears' THEN 'CRM_Contact' ELSE CONCAT(@client,' PC_SFDC Account' ) END);--updateme
...
LEFT JOIN (
	SELECT DISTINCT m.SSB_CRMSYSTEM_ACCT_ID
...
FROM dbo.vwDimCustomer_ModAcctID a 
LEFT JOIN (SELECT DISTINCT ssbid FROM dbo.vw_KeyAccounts) k
...
--    ) b     
--    ON a.SSID = b.ID
WHERE SourceSystem = 'crm_contact' AND a.SSB_CRMSYSTEM_CONTACT_ID IS NOT NULL
