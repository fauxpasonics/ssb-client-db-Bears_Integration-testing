...
WHERE SSB_CRMSYSTEM_Contact_ID IN (
	SELECT SSB_CRMSYSTEM_Contact_ID FROM dbo.vwDimCustomer_ModAcctId a
		WHERE SSB_CRMSYSTEM_Contact_ID IN (SELECT DISTINCT SSB_CRMSYSTEM_Contact_ID FROM dbo.vwDimCustomer_ModAcctId WHERE SourceSystem LIKE '%CRM_Contact') --updateme
	GROUP BY SSB_CRMSYSTEM_Contact_ID
HAVING COUNT(DISTINCT SourceSystem) = 1
) 
AND LoadType = 'Upsert'
