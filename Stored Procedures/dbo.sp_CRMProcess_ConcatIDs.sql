...
SELECT        CASE WHEN @ObjectType = 'Account' THEN ssb_crmsystem_contactacct_id ELSE [SSB_CRMSYSTEM_CONTACT_ID] END GUID, 
CAST(DimCustomerId AS VARCHAR(100)) AS DimCustomerID, CAST(SSID AS VARCHAR(50)) AS SSID
, a.SourceSystem
INTO #tmpDimCustIDs
...
,ISNULL(LEFT(STUFF((    SELECT  ', ' + CustomID1  AS [text()]
FROM (SELECT CustomID1,[GUID], MAX(CAST(Primary_Flag as int)) Primary_Flag 
		FROM stg.tbl_CRMProcess_NonWinners WHERE [SourceSystem] = 'TM-Suites' GROUP by CustomID1,[GUID]) acct
WHERE acct.[GUID] = z.[GUID] 
ORDER BY Primary_Flag desc, ', ' + CustomID1
...
, TMSuite_Ids = ISNULL(LTRIM([b].[ConcatIDs3]),'')
-- SELECT b.* 
